			...
			If StringStripWS(StringUpper($acommand[1]), 2) = "MOD" Then
				...
				Switch $modName
					Case "SMARTZAP"
						...
						Setlog("Executing " & $modName & " with min dark setting=" & $itxtDBLightMinDark)
						DEDropSmartSpell()
					Case Else
				...
				Switch $command
					...
					Case "DROP"
						...
						If StringInStr($value2, "%") > 0 Then
							$isIndexPercent = 1
							$index1 = Number(StringReplace($indexvect[0], "%", ""))
							If UBound($indexvect) > 1 Then
								$index2 = Number(StringReplace($indexvect[1], "%", ""))
							Else
								$index2 = $index1
							EndIf
						Else
						...
						If StringInStr($value3, "%") > 0 Then
							$isQtyPercent = 1
							$qty1 = Number(StringReplace($qtyvect[0], "%", ""))
							If UBound($qtyvect) > 1 Then
								$qty2 = Number(StringReplace($qtyvect[1], "%", ""))
							Else
								$qty2 = $qty1
							EndIf
						Else
